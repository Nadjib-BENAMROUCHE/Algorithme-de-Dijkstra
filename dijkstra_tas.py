...
#----------------------------------------------------------------------------------------------------
def parcour(dic,u,graphe,resulat):  
	global resultat 
		...
		if (tas !=[]): 
			...
			for i in range(len(graphe[u])):  
				...
				if (indice in dic.keys()): 
					if (dic[indice][0] > graphe[u][i][1] + dic[u][0]):
...
res= parcour(dic,3,graphe,resultat)
print("Fin de l'algorithme Le résultat est présenté sous forme [sommet, coût]:\n \n",res[1])
